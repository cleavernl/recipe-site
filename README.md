...
**Quick check:** in WSL, **`hostname -I`** (use the address Windows should forward to—often the first). On Windows (PowerShell or `cmd`), **`netsh interface portproxy show v4tov4`** — the row listening on **8000** must use that same address as **`connectaddress`**. If portproxy matches but Serve still returns **502**, from WSL run **`curl -fsS "http://$(hostname -I | awk '{print $1}'):8000/accounts/login/"`** (note **`:8000`** after the IP); if that fails while **`curl -fsS http://127.0.0.1:8000/accounts/login/`** works, use **`network_mode: host`** for the web service in **`compose.yaml`** (production default in this repo) so the app listens on the WSL eth0 address, not only inside the rootless port map.
